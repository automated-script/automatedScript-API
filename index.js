...
// ======================================================================
// ✅ GraphQL ADD-ON (ONLY NEW ENDPOINTS - REST APIs UNCHANGED)
// ======================================================================
 ...
 * /graphql:
 *   post:
 *     summary: GraphQL endpoint (Mock Users API)
 *     description: |
 *       This GraphQL endpoint is added ONLY for training purposes.
 *       It DOES NOT modify any REST API behaviour.
 *
 *       Supported operations:
 *       - query users(limit:Int, sort:SortOrder): returns status,count,data
 *       - query user(id:Int!): returns single user
 *       - query adminUsers: requires admin key -> else 403
 *       - query securePing: rate-limited demo -> 429
 *       - mutation createUser(name:String!, role:String!, id:Int): creates temporary user -> 201 + Location
 *       - mutation deleteUser(id:Int!): deletes temp created user -> 204 when x-no-content=true
 *
 *       Status codes supported (as per student Postman tests):
 *       200,201,204,304,400,401,403,404,405,415,422,429,500
 *     tags:
 *       - GraphQL
 *     requestBody:
 *       required: true
 ...
 *     responses:
 *       200: { description: OK }
 *       201: { description: Created (createUser mutation) }
 *       204: { description: No Content (deleteUser mutation when x-no-content=true) }
...
// 405 for GET /graphql (Postman expects Allow header POST)
...
app.post('/graphql', (req, res) => {
  try {
    // 500 simulate error header
    if (String(req.headers['x-simulate-error'] || '').toLowerCase() === 'true') {
    ...
    if (!body.query || typeof body.query !== 'string') {
      return badRequest(res, 'GraphQL field "query" is required');
    }
    ...
    // -----------------------------
    // This matches Postman "200 - Users OK" + ETag flow
    if (query.includes(' users') && !query.includes('adminusers') && !query.includes('secureping')) {
      let list = combinedUsers();

      // validate optional sort
      if (variables.sort !== undefined && variables.sort !== null) {
        const sort = String(variables.sort).toLowerCase();
        if (sort !== 'asc' && sort !== 'desc') return gqlError(res, 400, 'Invalid sort order');
        list.sort((a, b) => (sort === 'asc' ? a.id - b.id : b.id - a.id));
      }

      // validate optional limit
      if (variables.limit !== undefined && variables.limit !== null) {
        const n = Number(variables.limit);
        if (!Number.isInteger(n) || n <= 0) return gqlError(res, 400, 'Limit must be positive integer');
        list = list.slice(0, n);
      }
    ...
    // -----------------------------
    // ✅ QUERY: user(id:Int!) -> 200 / 400 / 401 / 404
    // -----------------------------
    if (query.includes('user(')) {
      const key = req.headers['x-api-key'];
      const expected = process.env.API_KEY || 'dev-key-123';
      if (!key || key !== expected) return gqlError(res, 401, 'Invalid or missing API key');

      const idNum = Number(variables.id);
      if (!Number.isInteger(idNum) || idNum <= 0) return gqlError(res, 400, 'id must be positive integer');

      const list = combinedUsers();
      const match = list.find(u => u.id === idNum);
      if (!match) return gqlError(res, 404, 'User not found');

      return res.status(200).json({ data: { user: match } });
    }

    // -----------------------------
    // ✅ QUERY: adminUsers -> 401/403/200
    // -----------------------------
    if (query.includes('adminusers')) {
      const key = req.headers['x-api-key'];
      if (!key) return gqlError(res, 401, 'Missing API key');
      if (key !== GRAPHQL_ADMIN_KEY) return gqlError(res, 403, 'Forbidden');
    ...
    // -----------------------------
    // ✅ QUERY: securePing -> 429 demo
    // -----------------------------
    if (query.includes('secureping')) {
      const ip = (req.headers['x-forwarded-for'] || req.socket.remoteAddress || '').split(',')[0].trim();
      ...
      return res.status(200).json({
        data: {
          securePing: { status: 200, message: 'OK' }
        }
      });
    }

    // -----------------------------
    // ✅ MUTATION: createUser -> 201 + Location
    // -----------------------------
    if (query.includes('mutation') && query.includes('createuser')) {
      const key = req.headers['x-api-key'];
      const expected = process.env.API_KEY || 'dev-key-123';
      if (!key || key !== expected) return gqlError(res, 401, 'Invalid or missing API key');

      const { name, role, id } = variables || {};
      if (name === undefined || typeof name !== 'string' || name.trim() === '') return gqlError(res, 400, 'Field "name" is required');
      if (role === undefined || typeof role !== 'string' || role.trim() === '') return gqlError(res, 400, 'Field "role" is required');

      let newId;
      if (id !== undefined && id !== null && String(id).trim() !== '') {
        const n = Number(id);
        if (!Number.isInteger(n) || n <= 0) return gqlError(res, 400, 'Field "id" must be positive integer');
        if (userExistsAnywhere(n)) return gqlError(res, 422, 'Field "id" already exists');
        newId = n;
      } else {
        newId = nextId();
      }

      // Create temporary user exactly like REST createUser does
      const expiresAt = now() + TTL_MS;
      const tempUser = { id: newId, name: String(name).trim(), role: String(role).trim(), expiresAt };
      tempUsers.set(newId, tempUser);
      setTimeout(() => tempUsers.delete(newId), TTL_MS);
    ...
    // -----------------------------
    // ✅ MUTATION: deleteUser -> 204 if x-no-content=true
    // -----------------------------
    if (query.includes('mutation') && query.includes('deleteuser')) {
      const key = req.headers['x-api-key'];
      const expected = process.env.API_KEY || 'dev-key-123';
      if (!key || key !== expected) return gqlError(res, 401, 'Invalid or missing API key');
      ...
      // only delete tempUsers as per postman behavior
      if (!tempUsers.has(idNum)) return gqlError(res, 404, 'User not found or expired');

      tempUsers.delete(idNum);

      const noContent = String(req.headers['x-no-content'] || '').toLowerCase() === 'true';
    ...
    // If nothing matched:
    return gqlError(res, 400, 'Unsupported GraphQL operation');

  } catch (e) {
    console.error('GraphQL unhandled error:', e);
    return res.status(500).json({ status: 500, error: 'Internal Server Error', message: 'GraphQL crashed' });
  }
});
